...
def update_config_file(filepath):
    ...
    If the values are missing, they are added to the end of the file.
    ...
    try:
        ...
        # If JetType or JetCone were not found, add them to the end of the file
        if not jet_type_found:
            updated_lines.append('JetType : "AK4PFchs"\n')
            needs_update = True
            print(f"Adding 'JetType : \"AK4PFchs\"' to {filepath}")
        
        if not jet_cone_found:
            updated_lines.append('JetCone : 0.4\n')
            needs_update = True
            print(f"Adding 'JetCone : 0.4' to {filepath}")
