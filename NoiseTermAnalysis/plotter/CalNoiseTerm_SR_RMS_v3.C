...
//   v_Syst.push_back("JECDown");
   for (int i= 0; i < 20; ++i)
   {
      if(i != 19){
         ...
         for (int isys = 0; isys < v_Syst.size(); isys++){
            ...
            //TString studyname = Form("StudySC2018_v6/MC/%s/EtaBin_%s_%s",v_Syst[isys].Data(),EtaBinsSt[i].Data(),EtaBinsSt[i+1].Data());
            //TString 
            studyname = Form("StudySC2018_v7p1/MCAll/%s/EtaBin_%s_%s",v_Syst[isys].Data(),EtaBinsSt[i].Data(),EtaBinsSt[i+1].Data());
            studyname = Form("Cone03_2018_v1/MCAll/%s/EtaBin_%s_%s",v_Syst[isys].Data(),EtaBinsSt[i].Data(),EtaBinsSt[i+1].Data());
            //TString studyname = Form("AppliedJEC_v5/MC/%s/EtaBin_%s_%s",v_Syst[isys].Data(),EtaBinsSt[i].Data(),EtaBinsSt[i+1].Data());
            ...
            hName = "h_Noise_Diff";
            //Calc(studyname,sample);
            Calc(studyname,sample, hName);
            hName = "h_Noise_Rho_Diff";
            Calc(studyname,sample, hName);

            studyname = Form("Cone03_2018_v1/PURunA/%s/EtaBin_%s_%s",v_Syst[isys].Data(),EtaBinsSt[i].Data(),EtaBinsSt[i+1].Data());
            hName = "h_Noise_Diff";
            Calc(studyname,sample, hName);
            hName = "h_Noise_Rho_Diff";
            Calc(studyname,sample, hName);
            studyname = Form("Cone03_2018_v1/PURunB/%s/EtaBin_%s_%s",v_Syst[isys].Data(),EtaBinsSt[i].Data(),EtaBinsSt[i+1].Data());
            hName = "h_Noise_Diff";
            Calc(studyname,sample, hName);
            hName = "h_Noise_Rho_Diff";
            Calc(studyname,sample, hName);
            studyname = Form("Cone03_2018_v1/PURunC/%s/EtaBin_%s_%s",v_Syst[isys].Data(),EtaBinsSt[i].Data(),EtaBinsSt[i+1].Data());
            hName = "h_Noise_Diff";
            Calc(studyname,sample, hName);
            hName = "h_Noise_Rho_Diff";
            Calc(studyname,sample, hName);
            studyname = Form("Cone03_2018_v1/PURunD/%s/EtaBin_%s_%s",v_Syst[isys].Data(),EtaBinsSt[i].Data(),EtaBinsSt[i+1].Data());
            hName = "h_Noise_Diff";
            Calc(studyname,sample, hName);
            hName = "h_Noise_Rho_Diff";
            Calc(studyname,sample, hName);
   ...
   for (int isys = 0; isys < v_Syst.size(); isys++){
      sample= "SingleNeutrino_Flat2018";
      studyname = Form("Cone03_2018_v1/MCAll/%s/AllEtaBin",v_Syst[isys].Data());
      hName = "h_Noise_Diff";
      Calc(studyname,sample, hName);
      hName = "h_Noise_Rho_Diff";
      Calc(studyname,sample, hName);
      studyname = Form("Cone03_2018_v1/PURunA/%s/AllEtaBin",v_Syst[isys].Data());
      hName = "h_Noise_Diff";
      Calc(studyname,sample, hName);
      hName = "h_Noise_Rho_Diff";
      Calc(studyname,sample, hName);
      studyname = Form("Cone03_2018_v1/PURunB/%s/AllEtaBin",v_Syst[isys].Data());
      hName = "h_Noise_Diff";
      Calc(studyname,sample, hName);
      hName = "h_Noise_Rho_Diff";
      Calc(studyname,sample, hName);
      studyname = Form("Cone03_2018_v1/PURunC/%s/AllEtaBin",v_Syst[isys].Data());
      hName = "h_Noise_Diff";
      Calc(studyname,sample, hName);
      hName = "h_Noise_Rho_Diff";
      Calc(studyname,sample, hName);
      studyname = Form("Cone03_2018_v1/PURunD/%s/AllEtaBin",v_Syst[isys].Data());
      hName = "h_Noise_Diff";
      Calc(studyname,sample, hName);
      hName = "h_Noise_Rho_Diff";
      Calc(studyname,sample, hName);
...
void Calc(TString StudyName, TString sample, TString hName)
{
   ...
   if (ftmp == NULL) cout << "No File!!" << endl;
      ...
      for (int i = 0; i < 9; ++i) 
      {
         ...
         //if (tmp != NULL) {SaveHist(tmp, -30,30, "p_{T}(R.C.1) - p_{T}(R.C.2) [GeV]",10, sample,StudyName);
