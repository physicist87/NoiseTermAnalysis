...
void NoiseTerm::Loop()
{
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) 
   {
      ...
      if (jentry % 100 == 0) printf("Event %lld\n", jentry); //%lld supports Long64_t
...
*/
      ...
      //cout << "isUseUserJEC : " << isUseUserJEC << endl;
...
RandCone NoiseTerm::SetRandomCone(double upperEta, double lowerEta)
{
   ...
   bool foundValidCones = false;

   while (!foundValidCones) 
   {
      NumOfSelct++;
      ...
      if (isPFInCone1 && isPFInCone2) {
         foundValidCones = true;
      }
   }/// End of while
...
RandCone NoiseTerm::SetRandomCone(double upperEta, double lowerEta)
{
   ...
   TLorentzVector SumRCone1, SumRCone2, rndmCone1, rndmCone2, JetCone1, JetCone2, clusterVectorE;

   double eta1 = -999, eta2 = -999, phi1 = -999, phi2 = -999;
   int NumOfSelct = 0;
   const int maxSelct = 1;

   double RangePhi = TMath::Pi() * 2.;
   double RangeEta = (upperEta - lowerEta) * 2;

   double depositERC1 = 0, depositERC2 = 0;
   double depositETRC1 = 0, depositETRC2 = 0;
   int num_rc1_pf = 0, num_rc2_pf = 0;

   bool foundValidCones = false;

   while (!foundValidCones && NumOfSelct < maxSelct) 
   {
      NumOfSelct++;
      phi1 = rand->Uniform(-1 * TMath::Pi(), TMath::Pi());
      phi2 = rand->Uniform(-1 * TMath::Pi(), TMath::Pi());
      ...
      double rndValue = rand->Rndm();
      eta1 = (rndValue < 0.5) ? -upperEta + rndValue * RangeEta : lowerEta + (rndValue - 0.5) * RangeEta;

      rndValue = rand->Rndm();
      eta2 = (rndValue < 0.5) ? -upperEta + rndValue * RangeEta : lowerEta + (rndValue - 0.5) * RangeEta;

      if (eta1 * eta2 > 0) eta2 = -eta2;
      ...
      for (unsigned int i = 0; i < pf_pt->size(); i++) 
      {
         if (!IsPFCHS(pf_type->at(i))) continue;

         clusterVectorE.SetPtEtaPhiE(pf_pt->at(i), pf_eta->at(i), pf_phi->at(i), pf_energy->at(i));

         bool tempInCone1 = false, tempInCone2 = false;
         if (fabs(rndmCone1.DeltaR(clusterVectorE)) < RCone) 
         {
            SumRCone1 += clusterVectorE;
            isPFInCone1 = true;
            tempInCone1 = true;
            depositERC1 += pf_energy->at(i);
            num_rc1_pf++;
      ...
      if (!overlayed_ && isPFInCone1 && isPFInCone2) 
      {
         foundValidCones = true;
      }
   }
   ...
   while (!foundValidCones) 
   {
      ...
      // Set the random cone properties
      rndmCone1.SetPtEtaPhiE(500, eta1, phi1, 500);
      rndmCone2.SetPtEtaPhiE(2000, eta2, phi2, 2000);
      ...
      // Loop through pf candidates and check if they are inside the cones
      for (unsigned int i = 0; i < pf_pt->size(); i++) 
      {
          if (!IsPFCHS(pf_type->at(i))) continue;
      ...
      // If particles are overlapping or not found in both cones, continue the loop
      if (overlayed_ || !(isPFInCone1 && isPFInCone2)) 
      {
          continue;
      }
   ...
   {
      tmp.findRC = true;
      tmp.diff_ = depositETRC1 - depositETRC2;
      JetCone1.SetPtEtaPhiE(depositETRC1, eta1, phi1, depositETRC1);
      JetCone2.SetPtEtaPhiE(depositETRC2, eta2, phi2, depositETRC2);
   }
...
/*RandCone NoiseTerm::SetRandomCone(double upperEta, double lowerEta)
{
    ...
    double eta1 = -999, eta2 = -999, phi1 = -999, phi2 = -999;
    int NumOfSelct = 0;
    const int MaxAttempts = 5000;  // Max attempts before adjusting the strategy
    ...
    while (!foundValidCones) 
    {
        ...
        // If many attempts fail, adjust the eta range to improve cone finding
        if (NumOfSelct > MaxAttempts) 
        {
            timeoutOccurred = true;
            std::cout << "Warning: Adjusting eta ranges after " << MaxAttempts << " attempts." << std::endl;
            // Adjust the eta range slightly to help find cones
            upperEta = upperEta * 0.95;  // Shrinking the range by 5%
            lowerEta = lowerEta * 1.05;  // Expanding the lower bound slightly
            RangeEta = (upperEta - lowerEta) * 2;
        }
    ...
    if (timeoutOccurred)
    {
        std::cout << "Warning: Valid random cones found after adjustments." << std::endl;
    }
